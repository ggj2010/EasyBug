    ...
    <head>
        ...
        $(document).ready(function(){
			...
			function view(id){
				 ...
				 var handler="${bugCountVO.userHandlerId}";
				 var status=id;
        ...
        <div id="main-div" class="width-p100">
            ...
            <div class="content main-page-190">
                ...
                <div class="margin-lr-1">
                	<form id="form1" action="${path}/main/count/bugContent/status.do" method="get">
                		...
                		<tr>
							<td class="ltd2" colspan="2">
								<div class="margin-lr-1">
									...
									<a class="btn btn-danger" href="javaScript:open('status');" >bug状态</a>
									...
									<a class="btn btn-default" href="javaScript:open('handler');" >Bug解决者</a>
				                    ...
				                    <c:if test='${fn:length(sumList)>0}'>
				                        <table class="list-tb" width="100%">
				                            <thead>
				                                <tr>
													<th> 
                                                    	<span id="biOrganNamseTitle">Bug状态</span>
													</a>
													</th>
													<th id="numberTh"> 
				                            ...
				                            <tbody>
				                                <c:forEach var="item" items='${sumList}' varStatus="s">
				                                    <tr>
														<td>
															<c:if test="${item[1]=='new'}">
                                            	    新录入
                                            </c:if>
                                            <c:if test="${item[1]=='open'}">
                                            	   已分配
                                            </c:if>
                                            <c:if test="${item[1]=='resolved'}">
                                            	   重新分配
                                            </c:if>
                                            <c:if test="${item[1]=='fixed'}">
                                            	    已解决
                                            </c:if>
                                            <c:if test="${item[1]=='solved'}">
                                            	    待审核
                                            </c:if>
                                            <c:if test="${item[1]=='repeate'}">
                                            	   重复的
                                            </c:if>
                                            <c:if test="${item[1]=='invalid'}">
                                            	  无效的
                                            </c:if>
											 <c:if test="${item[1]=='worksForMe'}">
                                            	  无法重新
                                            </c:if>
											 <c:if test="${item[1]=='later'}">
                                            	  延期处理
                                            </c:if>
				                                        </td>
