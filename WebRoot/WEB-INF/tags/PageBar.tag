	...
	<table style="width:100%;">
		<tr>
			...
			<td class="width_p70" style="text-align: right; padding-right:2px;">
				...
				<c:if test="${pageResult.currentPageNo >1 }">
					<a class="btn btn-default btn-lg" href="javascript:goPage(1);">首页</a>&nbsp;&nbsp;
				</c:if>
				<c:if test="${pageResult.hasPreviousPage}">
					<a class="btn btn-default btn-lg" href="javascript:goPage(${pageResult.currentPageNo-1});">上一页</a>&nbsp;&nbsp;
				</c:if>
				<c:if test="${!pageResult.hasPreviousPage}">
				  上一页&nbsp;&nbsp;
				</c:if>
				<c:if test="${pageResult.hasNextPage}">
					<a class="btn btn-default btn-lg" href="javascript:goPage(${pageResult.currentPageNo +1});">下一页</a>&nbsp;&nbsp;
				</c:if>
				<c:if test="${!pageResult.hasNextPage}">
				  下一页&nbsp;&nbsp;
				</c:if>
				...
				<c:if test="${pageResult.currentPageNo < pageResult.totalPageCount}">
				   <a class="btn btn-default btn-lg" href="javascript:goPage(${pageResult.totalPageCount});">末页</a>&nbsp;&nbsp;
				</c:if>
				&nbsp;&nbsp;
				个性化显示：
				...
				<a class="btn btn-default btn-lg" href="javascript:goPage($('#goPageNoTxt').val());">GO</a>
			</td>
		</tr>
	</table>
